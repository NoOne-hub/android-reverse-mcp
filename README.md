...
如果要启动 Ghidra sidecar 或完整 Ghidra 栈，还需要把以下文件放到仓库本地：
...
缺少这些文件时，`./scripts/run-ghidra.sh` 会在启动前直接报缺失文件，而不会等到 Docker build 中途才失败。

如果你本地网络慢，需要代理，请在运行命令前显式设置 `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY`；脚本默认不会假设别人机器上存在 `127.0.0.1:7890`。
...
```bash
./scripts/run-ghidra.sh /absolute/path/to/app.apk
./scripts/run-ida.sh /absolute/path/to/app.apk
```

只启动 sidecar 分析单个 `.so`：

```bash
./scripts/run-ghidra-sidecar.sh /absolute/path/to/libfoo.so
./scripts/run-ida-sidecar.sh /absolute/path/to/libfoo.so
```
...
./scripts/run-ghidra.sh /absolute/path/to/app.apk /absolute/path/to/workspace 8651
./scripts/run-ida.sh /absolute/path/to/app.apk /absolute/path/to/workspace 8651
```
...
- 默认把工作区放到仓库根目录下的 `.workspaces/`
- 自动创建持久化工作区
- 可从任意当前目录调用脚本；脚本会固定使用仓库内的 `docker-compose.yml`
- 前台运行，按 `Ctrl+C` 或脚本退出时自动执行 `docker compose down --remove-orphans`
- sidecar 脚本会把目标 `.so` 放到挂载工作区里，便于 IDA 直接打开或 Ghidra 后续 `program.open`
- IDA 启动脚本默认直接使用现成的 `android-reverse-mcp-ida-sidecar:latest`，不再要求本地重建 base image
- 启动脚本默认不注入代理；只有你显式设置 `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` 时才会透传给 Docker
